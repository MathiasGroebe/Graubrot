...
ogr2ogr -f $export_format $export_file PG:service=$pg_service -sql "SELECT fid, name, name_en, highway, railway, ref, service, tracktype, trail_visibility, surface, oneway, tunnel, bridge, layer, NULL::text AS label_wkt, NULL::numeric AS label_x, NULL::numeric AS label_y, NULL::integer AS label_rotation, TRUE::boolean AS label_visable, geom FROM $schema.traffic" -nln traffic -update
